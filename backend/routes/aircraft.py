...
def update_aircraft(aircraft_id: str, payload: schemas.AircraftUpdate, db: Session = Depends(get_db)):
    a = db.query(models.Aircraft).filter_by(AircraftID=aircraft_id).first()
    if not a:
        raise HTTPException(status_code=404, detail="Aircraft not found")
    update_data = payload.model_dump(exclude_unset=True)
    if "AirlineID" in update_data and update_data["AirlineID"] != a.AirlineID:
